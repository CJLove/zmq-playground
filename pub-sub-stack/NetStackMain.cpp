...
{
    std::cerr << "Usage:\n"
        << "net-stack -n <name> -f <configFile> -p <pubEndpoint> -s <subEndpoint> -S <sub topic> -P <pub topic> -d <destIP> -D <destPort>\n";
}
...
int main(int argc, char **argv)
{
    ...
    int c;
    while ((c = getopt(argc,argv,"f:n:l:p:s:P:S:h:d:D:L:?")) != EOF) {
        switch (c) {
        case 'f':
            configFile = optarg;
            break;
    ...
    NetStack stack(name, context, pubEndpoint, subEndpoint, subTopics, pubTopics, listenPort, destIp, destPort);
